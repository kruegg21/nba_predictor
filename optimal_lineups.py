				...
				if (list_1[i][0][0] != 0) & (list_2[j][0][0] != 0):
					...
					for l in combinations:
						if (l[0][0] != 0) & (l[1][0] != 0):
							...
							players = l[0][1] + l[1][1]
							teams = l[0][2] + l[1][2]
							ls[i+j].append([score, players, teams])
...
def optimal_lineup_test(df, column, cut_off, file_name, n_lineups):
	"""
	Inputs:
	...
	players_per_category = [2,2,2,2,1]
	...
	for i in xrange(num_categories):
		...
		for subset in itertools.combinations(cat[i].index, players_per_category[i]):
			...
			teams = [0] * players_per_category[i]
			salary = 0
			score = 0
			ctr = 0
			for k in subset:
				...
				teams[ctr] = cat[i].loc[k, 'Team']
			 	ctr += 1
				ls[i][int(salary)].append([score, players, teams])
	...
	best_lineups = lineups[:n_lineups]

	scores = [0] * len(best_lineups)
	players = [0] * len(best_lineups)
	teams = [0] * len(best_lineups)
