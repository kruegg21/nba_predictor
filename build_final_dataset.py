...
def build_final_dataset(player_data, team_data, full_dataset, first_time):
	...
	# LOAD:
	# load new data if this is not our first time
	...
	t1 = hf.time_checkpoint(t0, t0, "ADDED NUMERIC DATE")

	# error check after we have merged player and team data
	...
	t2 = hf.time_checkpoint(t0, t1, "MERGED TEAM AND PLAYER")
	...
	t3 = hf.time_checkpoint(t0, t2, "SHRUNK DATASETS")
	...
	t4 = hf.time_checkpoint(t0, t3, "CALCULATED ESTIMATED STATS PER POSSESSION PLAYED") 
	######################################################################################

	######################################################################################
	# for each game (THIS CAN LIKELY BE REPLACED WITH THE STAT PER POSSESSION PLAYED ABOVE)
	...
	t5 = hf.time_checkpoint(t0, t4, "CALCULATED PACE ADJUSTED STATS")
	######################################################################################
	...
	t6 = hf.time_checkpoint(t0, t5, "CALCULATED POSITION STATS")
	######################################################################################
	...
	final_dataset["FGAPerMin"] = ((final_dataset.FGA + 0.44 * final_dataset.FTA) * (final_dataset.TeamMP / 5)) / \
								 (final_dataset.MP * (final_dataset.TeamFGA + 0.44 * final_dataset.TeamFTA))

	final_dataset = hf.sort_and_reindex(final_dataset, ['Player','Date'])
	final_dataset = ga.get_averages(final_dataset, "FGAPerMin", "", "PlayerGameNumber", day_list, first_time)
	final_dataset["PercTeamShots"] = final_dataset.FGAPerMin
	...
	# calculate usage of each player
	...
	starters['Usg%'] = (starters.FGA + 0.44 * starters.FTA) / (starters.StarterFGA + 0.44 * starters.StarterFTA)

	starters = hf.calculate_game_number(starters, 'Player')
	starters = hf.sort_and_reindex(starters, ['Player','Date'])
	starters = ga.get_averages(starters, "Usg%", "", "PlayerGameNumber", day_list, first_time)

	column = 'L20Usg%'
	starters = hf.get_expected_usage(starters, column, 5)

	# add in expected stats and fill in empty cells (non-starters) with -999
	starters = starters[['Player', 'Date', 'Usg%', column, 'Expected' + column,
	...
	t7 = hf.time_checkpoint(t0, t6, "CALCULATED USAGE STATS")
	'''
	######################################################################################
